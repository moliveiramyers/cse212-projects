...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        else
        {
            for (int i = 0; i < letters.Length; i++)
            {
                ...
                var leftLetters = letters.Remove(i, 1);
                PermutationsChoose(results, leftLetters, size, word + currentLetter);
            }
        }
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        if (remember?.ContainsKey(s) == true)
        {
            return remember[s];
        }
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        char asterisk = '*';
        int index = pattern.IndexOf(asterisk);

        if (index != -1)
        {
            var wildcardCount = pattern.Count(c => c == asterisk);
